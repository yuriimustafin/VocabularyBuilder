...
export interface INewWordsClient {
    newWords_LookupWords(): Observable<string>;
}

@Injectable({
    providedIn: 'root'
})
export class NewWordsClient implements INewWordsClient {
    private http: HttpClient;
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    newWords_LookupWords(): Observable<string> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processNewWords_LookupWords(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processNewWords_LookupWords(response_ as any);
                } catch (e) {
    ...
    protected processNewWords_LookupWords(response: HttpResponseBase): Observable<string> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
